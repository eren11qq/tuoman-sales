"""
Interactive setup wizard for 拓漫 TouMan.
...
# display_hermes_home imported lazily at call sites (stale-module safety during tuoman update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ 拓漫 TouMan Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure 拓漫 TouMan using environment variables or config commands:")
    print_info("  tuoman config set model.provider custom")
    print_info("  tuoman config set model.base_url http://localhost:8080/v1")
    print_info("  tuoman config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'tuoman setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'tuoman setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'tuoman setup' to configure"))
    ...
    # Video generation — opt-in via `tuoman tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'tuoman setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'tuoman setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'tuoman setup terminal'"))
    ...
    # Spotify (OAuth via tuoman auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'tuoman setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('tuoman setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('tuoman setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('tuoman setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('tuoman setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('tuoman setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('tuoman config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('tuoman config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('tuoman config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('tuoman', Colors.GREEN)}              Start chatting")
    print(f"   {color('tuoman gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('tuoman doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `tuoman tools` and `tuoman setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``tuoman model``)
    ...
    provider added to ``tuoman model`` is automatically available here.
    ...
    # Delegate to the shared tuoman model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: tuoman model")
    ...
    # on demand via `tuoman auth add`, `tuoman setup` vision, and
    # `tuoman setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``tuoman model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via tuoman setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'tuoman setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where 拓漫 TouMan runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `tuoman setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `tuoman setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `tuoman setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where 拓漫 TouMan delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects 拓漫 TouMan to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  tuoman config edit")
    print_info("   Open config in your editor:  tuoman config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'tuoman setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     tuoman config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: tuoman gateway install")
            else:
                print_info("  You can install later: tuoman gateway install")
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: tuoman gateway install --system")
                print_info("  Or run in foreground:  tuoman gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   tuoman gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   tuoman gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `tuoman setup tools` and `tuoman tools` use the same flow:
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_hermes script as a module.
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure 拓漫 TouMan to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point 拓漫 TouMan at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point 拓漫 TouMan at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point 拓漫 TouMan at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point 拓漫 TouMan at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to 拓漫 TouMan equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing 拓漫 TouMan config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in 拓漫 TouMan.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ 拓漫 TouMan's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: tuoman claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: tuoman claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in 拓漫 TouMan (use tuoman claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``tuoman setup --portal`` and ``tuoman portal``. This is the
    ...
    拓漫 TouMan session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``tuoman setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``tuoman model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``tuoman portal`` always offers a model picker,
    ...
    print(color("│     ⚕ 拓漫 TouMan Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `tuoman portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `tuoman portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `tuoman portal`: %s", exc)
        print()
    ...
    print_info("  Run `tuoman portal info` to inspect routing.")
    print_info("  Run `tuoman` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      tuoman setup           — full or quick (auto-detected)
      tuoman setup model     — just model/provider
      tuoman setup tts       — just text-to-speech
      tuoman setup terminal  — just terminal backend
      tuoman setup gateway   — just messaging platforms
      tuoman setup tools     — just tool configuration
      tuoman setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ 拓漫 TouMan Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ 拓漫 TouMan Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your 拓漫 TouMan installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have 拓漫 TouMan configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'tuoman setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up 拓漫 TouMan?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'tuoman config edit'")
    ...
    # Tune later with `tuoman setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``tuoman setup <section>``
    or switch providers with ``tuoman model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'tuoman setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    tuoman setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  tuoman setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``tuoman tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``tuoman setup agent`` /
    ``tuoman config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `tuoman update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        tuoman tools")
        print_info("  Seed skills:         tuoman skills opt-in --sync")
        print_info("  Add MCP servers:     tuoman mcp add")
        print_info("  Enable plugins:      tuoman plugins")
        print_info("  Tune agent settings: tuoman setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`tuoman update` runs from re-injecting them. Opt back in any")
            print_info("time with `tuoman skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `tuoman tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `tuoman plugins list` / `tuoman plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `tuoman plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `tuoman mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `tuoman mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   tuoman tools")
    print_info("  Seed skills:         tuoman skills opt-in --sync")
    print_info("  Add MCP servers:     tuoman mcp add")
    print_info("  Tune agent settings: tuoman setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'tuoman setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'tuoman setup gateway'.")
