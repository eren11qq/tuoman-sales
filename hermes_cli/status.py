"""
Status command for tuoman CLI.

Shows the status of all 拓漫 TouMan components.
"""
...
def show_status(args):
    """Show status of all 拓漫 TouMan components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 拓漫 TouMan Status                   │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: tuoman portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: tuoman model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: tuoman auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: tuoman auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: tuoman model)"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   tuoman gateway")
    ...
    print(color("  Run 'tuoman doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'tuoman setup' to configure", Colors.DIM))
    print()
