...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or tuoman -c.",
    ...
    "Ctrl+Z suspends 拓漫 to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "tuoman -c resumes your most recent CLI session. tuoman -c \"project name\" resumes by title.",
    "tuoman -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "tuoman -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "tuoman chat -t web,terminal enables only specific toolsets for a focused session.",
    "tuoman chat -s github-pr-workflow preloads a skill at launch.",
    "tuoman chat -q \"query\" runs a single non-interactive query and exits.",
    "tuoman chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "tuoman chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "tuoman --yolo bypasses all dangerous command approval prompts for the entire session.",
    "tuoman chat --source telegram tags the session for filtering in tuoman sessions list.",
    "tuoman -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "tuoman doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "tuoman dump outputs a compact setup summary — great for bug reports.",
    "tuoman config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "tuoman config edit opens config.yaml in your default editor.",
    "tuoman config check scans for missing or stale configuration options.",
    "tuoman sessions browse opens an interactive session picker with search.",
    "tuoman sessions stats shows session counts by platform and database size.",
    "tuoman sessions prune --older-than 30 cleans up old sessions.",
    "tuoman skills search react --source skills-sh searches the skills.sh public directory.",
    "tuoman skills check scans installed hub skills for upstream updates.",
    "tuoman skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "tuoman skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "tuoman mcp add github --command npx adds MCP servers from the command line.",
    "tuoman mcp serve runs 拓漫 itself as an MCP server for other agents.",
    "tuoman auth add lets you add multiple API keys for credential pool rotation.",
    "tuoman completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "tuoman logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "tuoman backup creates a zip backup of your entire 拓漫 home directory.",
    "tuoman profile create coder creates an isolated profile that becomes its own command.",
    "tuoman profile create work --clone copies your current config and keys to a new profile.",
    "tuoman update syncs new bundled skills to ALL profiles automatically.",
    "tuoman gateway install sets up 拓漫 as a system service (systemd/launchd).",
    "tuoman memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "tuoman webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: tuoman tools disables unused tools, tuoman skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "tuoman models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as tuoman -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call 拓漫 tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'tuoman profile create coder' creates the 'coder' command.",
    "tuoman profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, 拓漫 prints a resume command with session ID and stats.",
    "tuoman sessions export backup.jsonl exports all sessions for backup or analysis.",
    "tuoman -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "tuoman skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: tuoman cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "拓漫 runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "tuoman gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to 拓漫 via a local macOS server.",
    ...
    "拓漫 loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "tuoman mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "tuoman mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "tuoman mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — tuoman tools can toggle them per platform.",
    ...
    "tuoman chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "tuoman plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "tuoman acp runs 拓漫 as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "tuoman auth add nous or tuoman auth add openai-codex sets up OAuth-based providers.",
    ...
    "tuoman status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make 拓漫 your own.",
    ...
    "Before a gateway session reset, 拓漫 auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope 拓漫 state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'tuoman profile create ls' would be rejected.",
    "tuoman profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "tuoman claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via tuoman tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — 拓漫 auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of 拓漫 config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while 拓漫 is working.',
    ...
    # --- Curator ---
    'tuoman curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "tuoman curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'tuoman curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'tuoman auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'tuoman dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'tuoman dashboard embeds the full 拓漫 TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'tuoman -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'tuoman chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'tuoman chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'tuoman chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "tuoman chat --source tool tags programmatic chats so they don't clutter tuoman sessions list.",
    'tuoman dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'tuoman sessions rename <ID> "new title" renames any past session; tuoman sessions delete <ID> removes one.',
    'tuoman import restores a full 拓漫 backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'tuoman fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'tuoman pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'tuoman setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'tuoman status --deep runs the full health sweep across every component; plain tuoman status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes 拓漫 block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from tuoman sessions list by default — set --source explicitly to see them.',
